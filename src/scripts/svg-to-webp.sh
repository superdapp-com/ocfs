...
for file in $(find ./dist -type f -name "*.svg"); do
    # Add --export-option=suppressCSSWarnings to ignore CSS warnings like mask-type
    inkscape --batch-process --export-type=png --export-width=256 --export-height=256 --export-option=suppressCSSWarnings $file -o ${file%.svg}.png
